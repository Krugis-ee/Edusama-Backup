                    ...
                    <div class="container-xxl flex-grow-1 container-p-y">
                        ...
                        <div class="app-ecommerce mb-4">
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
                                <div class="d-flex flex-column justify-content-center">
                                    <h4 class="mb-1 mt-3" style="color: <?php echo $org_color; ?>;">List Subjects</h4>
                                </div>
                                <?php
                                $show_add = 0;
                                $show_edit = 0;
                                ...
                                if (!in_array($user_role_id, $primary_roles)) {
                                    ...
                                    if (in_array('subjectsCreate', $privileges_arr)) {
                                        $show_add = 1;
                                    }
                                }
                                //Add
                                ...
                                <?php if ($show_add == 1) { ?>
                                    <div class="d-flex align-content-center flex-wrap gap-3">
                                        <div class="d-flex gap-3">
                                            <a href="{{ route('subject_add_get') }}">
                                                <button class="btn btn-primary" type="button">
                                                    <span>
                                                        <i class="ti ti-plus me-0 me-sm-1 ti-xs"></i>
                                                        <span class="d-none d-sm-inline-block">Add Subject</span>
                                                    </span>
                                                </button>
                                            </a>
                                        </div>
                        ...
                        <div class="card">
                            <div class="card-body table_admin text-nowrap">
                                ...
                                @endif
                                <input type="hidden" id="ajax_url" name="ajax_url" value="{{route('subject_change_status')}}">
                                <table id="example" class="display nowrap" style="width:100%">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            @if($subject->type == 1)
                                            <td><a data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="bottom" title="Click to Manage Subject" class="manage_subject" href="{{route('manage_subject',$subject->id)}}" class="class_integrations">{{ $subject->subject_name}}</a></td>
                                            @endif
                                            @if($subject->type ==2)
                                            <td><a data-bs-toggle="tooltip" data-popup="tooltip-custom" data-bs-placement="bottom" title="This Subject is suspended,please activate Subjects before assigning students." class="manage_subject" href="" class="class_integrations">{{ $subject->subject_name}}</td>
                                            @endif
                                            <!-- <td>{{$subject->org_name; }}</td> -->
                                            <td>
                                                <?php
                                                $branch_name = '';
                                                if ($subject->branch_id)
                                                    $branch_name = App\Models\Branch::getBranchNameByBranchId($subject->branch_id);
                                                echo $branch_name;
                                                ?></td>
                                            <td>
                                                ...
                                                if ($subject->type == 2) {
                                                ?>
                                                    <span class="badge rounded-pill bg-danger" title="{{$subject->suspend_msg; }}" style="cursor: pointer;">Suspended</span>
                                                <?php } ?>
                                            </td>
                                            <td>
                                                <?php if ($user_role_id == 1 || in_array('subjectsUpdate', $privileges_arr)) { ?>
                                                ...
                                                <?php }
                                                if ($user_role_id == 1 || in_array('subjectsDelete', $privileges_arr)) {
                                                ?>
                                                    <?php if ($subject->type == 1) { ?>
                                                        <button style="border: none;">
                                                            <span data-bs-toggle="tooltip" title="Suspend" style="cursor: pointer; background-color:#ea5455" data-bs-placement="bottom" id="suspend" class="badge badge-center toggle-class" data-id="{{ $subject->id }}" data-name="{{ $subject->subject_name}}" data-type="{{$subject->type}}">
                                                    ...
                                                    ?>
                                                        <button style="border: none;">
                                                            <span data-bs-toggle="tooltip" title="Activate" style="cursor: pointer; background-color:#28c76f" data-bs-placement="bottom" id="suspend" class="badge badge-center toggle-class" data-id="{{ $subject->id }}" data-name="{{ $subject->subject_name}}" data-type="{{$subject->type}}">
                                                                <i class="ti ti-droplet-exclamation"></i>
                                                            </span>
                                                        </button>
                                                <?php }
                                        ...
                                        <div class="modal fade" id="org_suspend" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{route('subject_change_status')}}" method="POST">
                                                        @csrf
