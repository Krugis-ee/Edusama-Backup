                ...
                <div class="content-wrapper">
                    ...
                    <div class="container-xxl flex-grow-1 container-p-y" id="student_attendances">
                        <div class="app-ecommerce mb-3">
                            <form method="post" action="{{route('teacher_edit_attendance_process')}}">
                                @csrf
                                ...
                                <div class="card-header">
                                    ...
                                    <div class="row mb-4 mt-4">
                                        <div class="d-flex align-content-center flex-wrap col-3">
                                            ...
                                            <input type="text" class="form-control date" placeholder="DD-MM-YYYY" id="flatpickr-date" name="date" />
                                            <input type="hidden" name="class_room_id" value="{{$class_room_id}}" class="class_room_id">
                                            <input type="hidden" name="subject_id" value="{{$subject_id}}" class="subject_id">
                                            <input type="hidden" name="students_id" value="{{$students_id}}" class="students_id">
                                            <input type="hidden" name="teacher_id" value="{{$teacher_id}}" class="teacher_id">
                                        </div>
                                        <div class="col-2">
                                ...
                                <div class="grid" id="attendance_grid">
                                    <div class="grid-container">
                                        <table id="student_attendance" class="students_list">
                                            <thead>
                                                <tr class="header">
                                                    <th>Student Name
    ...
    <script>
        $(document).ready(function() {
            ...
            $("#show_attendance").click(function() {
                ...
                $(".students_list .table_row").remove();
                ...
                $.ajax({
                    url: '{{ route("teacher_list_attendance") }}',
                    type: 'GET',
                    data: {
                    ...
                    success: function(response) {
                        ...
                        $.each(response['attendance'], function(i, item) {
                            console.log(item);
                            if (item.attendance_score == 1) {
                                var present = "checked";
                            } else {
                                var present = "";
                            }
                            if (item.attendance_score == 0) {
                                var absent = "checked";
                            } else {
                                var absent = "";
                            }
                            if (item.attendance_score == 0.5) {
                                var half_day = "checked";
                            } else {
                                var half_day = "";
                            }

                            var rows = "<tr class='table_row'>" + '<td>' + item.first_name + " " + item.last_name + '</td>' +
                                '<td class="form-check-success"><label><input type="checkbox" class="radio form-check-input" value="1.0" name="attendance[' + j + ']" id="customCheckSuccess" ' + present + '/>&nbsp;Present</label></td>' +
                                '<td class="form-check-danger"><label><input type="checkbox" class="radio form-check-input" value="0.0" name="attendance[' + j + ']" id="customCheckDanger" ' + absent + '/>&nbsp;Absent</label></td>' +
                                '<td class="form-check-warning"><label><input type="checkbox" class="radio form-check-input" value="0.5" name="attendance[' + j + ']" id="customCheckWarning" ' + half_day + '/>&nbsp;Half Day</label></td>' +
                                "</tr>"
                            j++;
                            $('.students_list tbody').append(rows);
                        })
                    }
                })
