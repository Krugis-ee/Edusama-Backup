...
<html
   ...
   <body class="add_admin">
                 ...
                 <div class="app-ecommerce mb-4">
                            <div class="d-flex flex-column flex-md-row justify-content-between align-items-start align-items-md-center">
                                <div class="d-flex flex-column justify-content-center">
                                    <h4 class="mb-1 mt-3" style="color: <?php echo $org_color; ?>;">List Parents</h4>
                                </div>
								<?php
								$show_add=0;
								$show_edit=0;
    ...
    if (!in_array($user_role_id, $primary_roles)) {
		...
		if (in_array('parentsCreate', $privileges_arr)) {
	$show_add=1;
	}}
	//Add
                                ...
                                <div class="d-flex align-content-center flex-wrap gap-3">
                                    <div class="d-flex gap-3">
                                        <a href="{{ route('parent_add') }}">
                                            <button class="btn btn-primary" type="button">
                                                <span>
                                                    <i class="ti ti-plus me-0 me-sm-1 ti-xs"></i>
                                                    <span class="d-none d-sm-inline-block">Add Parent</span>
                                                </span>
                                            </button>
                                        </a>
                                    </div>
                 ...
                 <div class="card">
                    <div class="card-body table_admin text-nowrap">
                     ...
                     @if(session()->has('message'))
                                ...
                                @endif
                     <input type="hidden" id="ajax_url" name="ajax_url" value="{{route('change_parent_status')}}">
                     <?php
                     $user_id=session()->get('loginId');
                     $privileges=App\Models\UserPrivilege::getPrivilegesByUserId($user_id);
                     ...
                     <table id="example" class="display nowrap" style="width:100%">
                          ...
                          <tbody>
                              <?php foreach ($parents as $parent) {
                                 ...
                                 echo '<tr>';
                                 echo '<td>' . $parent->first_name . '</td>';
                                 echo '<td>' . $parent->last_name . '</td>';
                                 echo '<td>' . $parent->email . '</td>';
                                 echo '<td>' . $parent->mobile_number . '</td>';
                                 $branch_name = App\Models\Branch::getBranchNameByBranchId($parent->branch_id);
                                 echo '<td>' . $branch_name . '</td>';
                                 ?>
								 <td>
                                                <?php if ($parent->type == 1) { ?>
                                                    <span class="badge rounded-pill bg-success">Active</span>
                                                <?php }
                                                if ($parent->type == 2) {
                                                ?>
                                                    <span class="badge rounded-pill bg-danger" title="{{$parent->suspend_msg}}" style="cursor: pointer;">Suspended</span>
                                                <?php } ?>
                                            </td>
                                 <td>
                                    <?php if($user_role_id==1 || in_array('parentsUpdate',$all_privilege_arr)) {?>
                                     <a href="{{ route("parent_edit", $parent->id) }}" >
									 <span data-bs-toggle="tooltip" data-bs-placement="bottom" title="Edit" class="badge badge-center bg-warning">
                                                        <i class="ti ti-edit"></i>
                                                    </span>
									 </a>

                                     <?php } if($user_role_id==1 || in_array('parentsDelete',$all_privilege_arr)) {
										 if ($parent->type == 1) { ?>
                                                <button style="border: none;">
                                                    <span data-bs-toggle="tooltip" title="Suspend" style="cursor: pointer; background-color:#ea5455" data-bs-placement="bottom" id="suspend" class="badge badge-center toggle-class"  data-id="{{ $parent->id }}" data-name="{{ $parent->first_name.' '.$parent->last_name }}" data-type="{{$parent->type}}">
                                                ...
                                                ?>
                                                <button style="border: none;">
                                                    <span data-bs-toggle="tooltip" title="Activate" style="cursor: pointer; background-color:#28c76f" data-bs-placement="bottom" id="suspend" class="badge badge-center toggle-class"  data-id="{{ $parent->id }}" data-name="{{ $parent->first_name.' '.$parent->last_name }}" data-type="{{$parent->type}}">
                                                        <i class="ti ti-droplet-exclamation"></i>
                                                    </span>
                                                </button>
                                                <?php }
                               ...
                               <div class="modal fade" id="org_suspend" tabindex="-1" aria-hidden="true">
                                            <div class="modal-dialog modal-dialog-centered" role="document">
                                                <div class="modal-content">
                                                    ...
                                                    <form action="{{route('change_parent_status')}}" method="POST">
                                                        @csrf
